...
Agent *createAgent(Canvas *canvas, const char *type, char symbol, int is_predator) {
    ...
    agent->nn = NN_create(5, 20, 4, NULL, NULL, NULL, NULL, 0.01, 0.9);
    if (!agent->nn) {
...
void crossover(NN_t *parent1, NN_t *parent2, NN_t *child) {
    for (int i = 0; i < parent1->numWeights; i++) {
...
void evolvePopulation(Simulation *simulation) {
    ...
    for (int i = MAX_PREDATORS / 2; i < MAX_PREDATORS; i++) {
        int parent1 = rand() % (MAX_PREDATORS / 2);
        int parent2 = rand() % (MAX_PREDATORS / 2);
    ...
    for (int i = MAX_PREY / 2; i < MAX_PREY; i++) {
        int parent1 = rand() % (MAX_PREY / 2);
        int parent2 = rand() % (MAX_PREY / 2);
